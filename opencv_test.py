...
canvases = [100*np.ones((canvas_size, canvas_size, 3), dtype="uint8") for i in range(4)]
...
def warp():
    ...
    for count, canvas in enumerate(canvases):
        ...
        new_canvas[stride*(3-count):stride*(3-count) + h_warp,:,:] += (dst*0.7).astype("uint8")

def plot(currentBoardBlack, currentBoardWhite):
    global canvases, new_canvas
    canvases = [100*np.ones((canvas_size, canvas_size, 3), dtype="uint8") for i in range(4)]
    new_canvas = np.zeros((int(canvas_size+h_margin), int(canvas_size+w_margin), 3), dtype="uint8")
